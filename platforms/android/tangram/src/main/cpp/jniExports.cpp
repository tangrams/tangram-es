...
extern "C" {
    ...
    JNIEXPORT void JNICALL Java_com_mapzen_tangram_MapController_nativeSetPositionZoomRotationTilt(JNIEnv* jniEnv, jobject obj, jlong mapPtr, jdouble lon, jdouble lat, jfloat zoom, jfloat radiansRotation, jfloat radiansTilt) {
        assert(mapPtr > 0);
        auto map = reinterpret_cast<Tangram::Map*>(mapPtr);
    ...
    JNIEXPORT void JNICALL Java_com_mapzen_tangram_MapController_nativeCancelEasing(JNIEnv* jniEnv, jobject obj, jlong mapPtr) {
        assert(mapPtr > 0);
        auto map = reinterpret_cast<Tangram::Map*>(mapPtr);
        map->clearEasing();
    }
    ...
    JNIEXPORT void JNICALL Java_com_mapzen_tangram_MapController_nativeSetDataSourceUrl(JNIEnv* jniEnv, jobject obj, jlong mapPtr, jstring dataSourceName, jstring url) {
        assert(mapPtr > 0);
        auto map = reinterpret_cast<Tangram::Map*>(mapPtr);
        auto dataSourceNameString = stringFromJString(jniEnv, dataSourceName);
        auto urlString = stringFromJString(jniEnv, url);
        map->setDataSourceUrl(dataSourceNameString.c_str(), urlString.c_str());
    }
