...
UrlRequestHandle AndroidPlatform::startUrlRequest(Url _url, UrlCallback _callback) {
...

    {
        ...
        requestHandle = m_urlRequestCount++;

        if (!_url.hasHttpScheme()) {
            m_requests.push_back({requestHandle, _url, _callback});
    ...
    assert(requestHandle == static_cast<UrlRequestHandle>(static_cast<jlong>(requestHandle)));
