...
SimpleMesh::SimpleMesh():m_drawMode(GL_TRIANGLES) {
    
}

SimpleMesh::SimpleMesh(const SimpleMesh &_mother):m_drawMode(_mother.getDrawMode()) {
    add(_mother);
}
...
bool SimpleMesh::load(const std::string& _file) {
...
bool SimpleMesh::save(const std::string& _file, bool _useBinary) {
...
void SimpleMesh::setDrawMode(GLenum _drawMode) {
    m_drawMode = _drawMode;
}

void SimpleMesh::setColor(const glm::vec4 &_color) {
    m_colors.clear();
...
void SimpleMesh::addColor(const glm::vec4 &_color) {
    m_colors.push_back(_color);
}

void SimpleMesh::addColors(const std::vector<glm::vec4> &_colors) {
    m_colors.insert(m_colors.end(), _colors.begin(), _colors.end());
}

void SimpleMesh::addVertex(const glm::vec3 &_point){
   m_vertices.push_back(_point);
}

void SimpleMesh::addVertices(const std::vector<glm::vec3>& _verts){
   m_vertices.insert(m_vertices.end(),_verts.begin(),_verts.end());
}

void SimpleMesh::addVertices(const glm::vec3* verts, int amt){
   m_vertices.insert(m_vertices.end(),verts,verts+amt);
}

void SimpleMesh::addNormal(const glm::vec3 &_normal){
    m_normals.push_back(_normal);
}

void SimpleMesh::addNormals(const std::vector<glm::vec3> &_normals ){
    m_normals.insert(m_normals.end(), _normals.begin(), _normals.end());
}

void SimpleMesh::addTexCoord(const glm::vec2 &_uv){
    m_texCoords.push_back(_uv);
}

void SimpleMesh::addTexCoords(const std::vector<glm::vec2> &_uvs){
    m_texCoords.insert(m_texCoords.end(), _uvs.begin(), _uvs.end());
}

void SimpleMesh::addIndex(uint16_t _i){
    m_indices.push_back(_i);
}

void SimpleMesh::addIndices(const std::vector<uint16_t>& inds){
	m_indices.insert(m_indices.end(),inds.begin(),inds.end());
}

void SimpleMesh::addIndices(const uint16_t* inds, int amt){
	m_indices.insert(m_indices.end(),inds,inds+amt);
}

void SimpleMesh::addTriangle(uint16_t index1, uint16_t index2, uint16_t index3){
    addIndex(index1);
    addIndex(index2);
    addIndex(index3);
}

void SimpleMesh::add(const SimpleMesh &_mesh){
...
GLenum SimpleMesh::getDrawMode() const{
    return m_drawMode;
}

const std::vector<glm::vec4> & SimpleMesh::getColors() const{
    return m_colors;
}

const std::vector<glm::vec3> & SimpleMesh::getVertices() const{
	return m_vertices;
}

const std::vector<glm::vec3> & SimpleMesh::getNormals() const{
    return m_normals;
}

const std::vector<glm::vec2> & SimpleMesh::getTexCoords() const{
    return m_texCoords;
}

const std::vector<uint16_t> & SimpleMesh::getIndices() const{
    return m_indices;
}

std::vector<glm::ivec3> SimpleMesh::getTriangles() const {
