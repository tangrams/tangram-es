...
int main(int argc, char **argv){
    ...
    Tangram::initialize();
    setViewport(0,0,state->screen_width, state->screen_height);
...
void onMouseDrag() {
    if( mouse.button == 1 ){
        ...
        if ( keyPressed == 'r') {
            float scale = -0.05;
            float rot = atan2(mouse.velY,mouse.velX);
            if( mouse.x < viewport.width/2.0 ) {
                scale *= -1.0;
            }
            Tangram::handleRotateGesture(state->screen_width/2.0, state->screen_height/2.0, rot*scale);
        } else if ( keyPressed == 't') {
            Tangram::handleShoveGesture(mouse.velY*0.005);
        } else {
...
void onViewportResize(int _newWidth, int _newHeight) {
    Tangram::resize(viewport.width, viewport.height);
    requestRender();
}
