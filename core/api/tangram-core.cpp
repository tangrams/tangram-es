...
bool startUrlRequest(void* context, const std::string& _url, UrlCallback _callback) {
    if (tangram_url_fetcher) {
        return tangram_url_fetcher(context, _url.c_str(), _callback);
    } else {
        return false;
    }
}
...
TANGRAM_CORE_EXPORT TangramTouchItemArray tangramPickFeaturesAt(tangram_map_t map, float _x, float _y) {
    TangramPointerArray planeFeatures = { nullptr, 0 };
    auto &features = ((Tangram::Map*)map)->pickFeaturesAt(_x, _y);
