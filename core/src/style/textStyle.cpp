...
void TextStyle::onBeginDrawFrame(const View& _view, Scene& _scene, int _textureUnit) {
    ...
    m_shaderProgram->setUniformf(Uniform::uvScaleFactor,
                                 1.0f / m_fontContext->getAtlasResolution());
    m_shaderProgram->setUniformi(Uniform::tex, 0);
    m_shaderProgram->setUniformMatrix4f(Uniform::ortho, _view.getOrthoViewportMatrix());
