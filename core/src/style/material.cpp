...
void Material::setupProgram(ShaderProgram& _shader) {
    ...
    if (m_bEmission) {
        _shader.setUniformf(Uniform::materialEmission, m_emission);

        if (m_emission_texture.tex) {
            m_emission_texture.tex->update(1);
            m_emission_texture.tex->bind(1);
            _shader.setUniformi(Uniform::materialEmissionTexture, 1);
            _shader.setUniformf(Uniform::materialEmissionScale, m_emission_texture.scale);
        }
    }

    if (m_bAmbient) {
        _shader.setUniformf(Uniform::materialAmbiant, m_ambient);

        if (m_ambient_texture.tex) {
            m_ambient_texture.tex->update(2);
            m_ambient_texture.tex->bind(2);
            _shader.setUniformi(Uniform::materialAmbiantTexture, 2);
            _shader.setUniformf(Uniform::materialAmbiantScale, m_ambient_texture.scale);
        }
    }

    if (m_bDiffuse) {
        _shader.setUniformf(Uniform::materialDiffuse, m_diffuse);

        if (m_diffuse_texture.tex) {
            m_diffuse_texture.tex->update(3);
            m_diffuse_texture.tex->bind(3);
            _shader.setUniformi(Uniform::materialDiffuseTexture, 3);
            _shader.setUniformf(Uniform::materialDiffuseScale, m_diffuse_texture.scale);
        }
    }

    if (m_bSpecular) {
        _shader.setUniformf(Uniform::materialSpecular, m_specular);
        _shader.setUniformf(Uniform::materialShininess, m_shininess);

        if (m_specular_texture.tex) {
            m_specular_texture.tex->update(4);
            m_specular_texture.tex->bind(4);
            _shader.setUniformi(Uniform::materialSpecularTexture, 4);
            _shader.setUniformf(Uniform::materialSpecularScale, m_specular_texture.scale);
        }
    }
    ...
    if (m_normal_texture.tex) {
        ...
        _shader.setUniformi(Uniform::materialNormalTexture, 5);
        _shader.setUniformf(Uniform::materialNormalScale, m_normal_texture.scale);
        _shader.setUniformf(Uniform::materialNormalAmount, m_normal_texture.amount);
    }
}
