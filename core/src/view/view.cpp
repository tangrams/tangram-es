...
double invLodFunc(double d) { return exp2(d) - 1.0; }
...
void View::setMapProjection(ProjectionType _projType) {
    ...
    switch (_projType) {
    case ProjectionType::mercator: m_projection.reset(new MercatorProjection()); break;
    default:
...
const MapProjection& View::getMapProjection() const { return *m_projection.get(); }
...
void View::translate(double _dx, double _dy) { setPosition(m_pos.x + _dx, m_pos.y + _dy); }

void View::zoom(float _dz) { setZoom(m_zoom + _dz); }

void View::roll(float _droll) { setRoll(m_roll + _droll); }

void View::pitch(float _dpitch) { setPitch(m_pitch + _dpitch); }
...
void View::update() {
    ...
    if (!Tangram::getDebugFlag(Tangram::DebugFlags::freeze_tiles)) { updateTiles(); }
...
float View::screenToGroundPlane(float& _screenX, float& _screenY) const {
    ...
    glm::vec4 ray_clip = {2.f * _screenX / m_vpWidth - 1.f, 1.f - 2.f * _screenY / m_vpHeight, -1.f,
                          1.f};                                // Ray from camera in clip space
    ...
    if (ray_world.z != 0.f) { t = -m_pos.z / ray_world.z; }
    ...
    if (rayDistanceXY > maxTileDistance || t < 0) { ray_world *= maxTileDistance / rayDistanceXY; }
...
void View::updateMatrices() {
    ...
    if (m_width > m_height) { fovy /= m_aspect; }
    ...
    // The matrix that transforms normals from world space to camera space is the transpose of the inverse of the view
    // matrix,
...
void View::updateTiles() {
    ...
    if (t0 < .0f && t1 < 0.f && t2 < 0.f && t3 < 0.f) { return; }
