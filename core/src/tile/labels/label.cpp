...
Label::Label(Label::Transform _transform, std::string _text, fsuint _id, Type _type)
    : m_type(_type), m_transform(_transform), m_text(_text), m_id(_id) {
...
void Label::pushTransform(std::shared_ptr<TextBuffer>& _buffer) {
    if (m_dirty) {
        _buffer->transformID(m_id, m_transform.m_screenPosition.x, m_transform.m_screenPosition.y,
...
bool Label::updateScreenTransform(const glm::mat4& _mvp, const glm::vec2& _screenSize) {
    ...
    case Type::line: {
        ...
        if (m_dim.x > length) {
            ...
            if (exceed > exceedHeuristic) { return false; }
        }
...
bool Label::offViewport(const glm::vec2& _screenSize) {
    ...
    for (int i = 0; i < 4; ++i) {
        const auto& p = quad[i];
        if (p.x < _screenSize.x && p.y < _screenSize.y && p.x > 0 && p.y > 0) { return false; }
    }
...
void Label::occlusionSolved() { m_occlusionSolved = true; }
...
void Label::updateState(const glm::mat4& _mvp, const glm::vec2& _screenSize, float _dt) {
    ...
    if (offViewport(_screenSize)) { enterState(State::out_of_screen, 0.0); }
    ...
    case State::fading_in:
        ...
        s_needUpdate = true;
        if (m_fade.isFinished()) enterState(State::visible, 1.0);
        break;
    case State::fading_out:
        setAlpha(m_fade.update(_dt));
        s_needUpdate = true;
        if (m_fade.isFinished()) enterState(State::sleep, 0.0);
        break;
    case State::out_of_screen:
        if (!offViewport(_screenSize)) enterState(State::wait_occ, 0.0);
        break;
    case State::wait_occ:
        ...
        } else if (occludedLastFrame && m_occlusionSolved) { enterState(State::sleep, 0.0); }
        break;
    case State::sleep:
        ;
        // dead state
