    ...
    while (_geomIn.getData() < _geomIn.getEnd()) {
        ...
        if (cmd == pbfGeomCmd::moveTo || cmd == pbfGeomCmd::lineTo) { // get parameters/points
            ...
            if (cmd == pbfGeomCmd::moveTo) {
                if (line.size() > 0) { _out.push_back(line); }
                line.clear();
            }
        ...
        } else if (cmd == pbfGeomCmd::closePath) { // end of a polygon, push first point in this line as last and push
                                                   // line to poly
...
void PbfParser::extractFeature(protobuf::message& _featureIn, Feature& _out, const MapTile& _tile,
                               std::vector<std::string>& _keys, std::vector<float>& _numericValues,
                               std::vector<std::string>& _stringValues, int _tileExtent) {
    ...
    while (_featureIn.next()) {
        ...
        case 2: {
            ...
            while (tagsMsg) {
                ...
                } else { _out.props.stringProps[key] = strVal; }
            }
            break;
        }
        // Feature Type
    ...
    case GeometryType::points:
        for (auto& line : geometryLines) {
            for (auto& point : line) { _out.points.emplace_back(point); }
        }
        break;
    case GeometryType::lines:
        for (auto& line : geometryLines) { _out.lines.emplace_back(line); }
        break;
    case GeometryType::polygons: _out.polygons.emplace_back(geometryLines); break;
    case GeometryType::unknown: break;
    default: break;
    }
}
