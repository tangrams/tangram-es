...
SpotLight::SpotLight(const std::string& _name, bool _dynamic)
    : PointLight(_name, _dynamic), m_direction(1.0, 0.0, 0.0), m_spotExponent(0.0), m_spotCutoff(0.0),
      m_spotCosCutoff(0.0) {
...
void SpotLight::setDirection(const glm::vec3& _dir) { m_direction = _dir; }
...
void SpotLight::setCutoffExponent(float _exponent) { m_spotExponent = _exponent; }
...
void SpotLight::setupProgram(const std::shared_ptr<View>& _view, std::shared_ptr<ShaderProgram> _shader) {
    if (m_dynamic) {
        ...
        if (m_origin == LightOrigin::world) { direction = glm::normalize(_view->getNormalMatrix() * direction); }
...
std::string SpotLight::getClassBlock() {
    if (s_classBlock.empty()) { s_classBlock = stringFromResource("spotLight.glsl") + "\n"; }
    return s_classBlock;
}
...
const std::string& SpotLight::getTypeName() { return s_typeName; }
