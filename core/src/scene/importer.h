...
public:
    ...
    Node applySceneImports(const std::shared_ptr<const Platform>& platform, const Url& scenePath, const Url& resourceRoot = Url());

// protected for testing purposes, else could be private
protected:
    virtual std::string getSceneString(const std::shared_ptr<const Platform>& platform, const Url& scenePath);
