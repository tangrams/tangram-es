...
std::string parseSequence(const Node& node) {
    ...
    for (auto& val : node) { sstream << val.as<float>() << ","; }
    return sstream.str();
}
...
void SceneLoader::loadSources(Node sources, TileManager& tileManager) {
    ...
    for (auto it = sources.begin(); it != sources.end(); ++it) {
        ...
        if (type == "GeoJSONTiles") {
            ...
            // TODO
        } else if (type == "MVT") { sourcePtr = std::unique_ptr<DataSource>(new MVTSource(name, url)); }

        if (sourcePtr) { tileManager.addDataSource(std::move(sourcePtr)); }
    }
}
...
void SceneLoader::loadLights(Node lights, Scene& scene) {
    ...
    for (auto it = lights.begin(); it != lights.end(); ++it) {
        ...
        if (type == "ambient") {
            ...
            if (direction) { dLightPtr->setDirection(parseVec3(direction)); }
            ...
            if (position) { pLightPtr->setPosition(parseVec3(position)); }
            Node radius = light["radius"];
            if (radius) {
                if (radius.size() > 1) { pLightPtr->setRadius(radius[0].as<float>(), radius[1].as<float>()); } else {
                    pLightPtr->setRadius(radius.as<float>());
                }
            }
            Node att = light["attenuation"];
            if (att) { pLightPtr->setAttenuation(att.as<float>()); }
            ...
            Node position = light["position"];
            if (position) { sLightPtr->setPosition(parseVec3(position)); }
            Node direction = light["direction"];
            if (direction) { sLightPtr->setDirection(parseVec3(direction)); }
            Node radius = light["radius"];
            if (radius) {
                if (radius.size() > 1) { sLightPtr->setRadius(radius[0].as<float>(), radius[1].as<float>()); } else {
                    sLightPtr->setRadius(radius.as<float>());
                }
            }
            Node angle = light["angle"];
            if (angle) { sLightPtr->setCutoffAngle(angle.as<float>()); }
            Node exponent = light["exponent"];
            if (exponent) { sLightPtr->setCutoffExponent(exponent.as<float>()); }
        ...
        if (origin) {
            ...
            if (originStr == "world") { lightPtr->setOrigin(LightOrigin::world); } else if (originStr == "camera") {
                lightPtr->setOrigin(LightOrigin::camera);
            } else if (originStr == "ground") { lightPtr->setOrigin(LightOrigin::ground); }
        }

        Node ambient = light["ambient"];
        if (ambient) { lightPtr->setAmbientColor(parseVec4(ambient)); }

        Node diffuse = light["diffuse"];
        if (diffuse) { lightPtr->setDiffuseColor(parseVec4(diffuse)); }

        Node specular = light["specular"];
        if (specular) { lightPtr->setSpecularColor(parseVec4(specular)); }
...
void SceneLoader::loadCameras(Node cameras, View& view) {
    ...
    for (auto it = cameras.begin(); it != cameras.end(); ++it) {
        ...
        if (position) {
            ...
            if (position.size() > 2) { z = position[2].as<float>(); }
        }
...
Filter* SceneLoader::generateFilter(YAML::Node _filter) {
    ...
    for (YAML::const_iterator filtItr = _filter.begin(); filtItr != _filter.end(); ++filtItr) {
        ...
        } else if (filtItr->first.as<std::string>() == "all") { filter = generateFilter(_filter["all"]); } else {
    ...
    if (filters.size() == 1) { return filters.front(); } else if (filters.size() > 0) {
...
Filter* SceneLoader::generatePredicate(YAML::Node _node, std::string _key) {
    ...
    if (_node.IsScalar()) {
        try {
            ...
            if (value == "true") { return (new Existence(_key, true)); } else if (value == "false") {
                return (new Existence(_key, false));
            } else { return (new Equality(_key, {new StrValue(value)})); }
        }
...
void SceneLoader::parseStyleProps(Node styleProps, StyleParamMap& paramMap, const std::string& propPrefix) {
    ...
    for (auto propItr = styleProps.begin(); propItr != styleProps.end(); ++propItr) {
        ...
        std::string paramKey;
        if (propPrefix.length() > 0) { paramKey = propPrefix + ":" + prop.as<std::string>(); } else {
        ...
        } else if (propItr->second.IsMap()) { parseStyleProps(propItr->second, paramMap, paramKey); } else {
            logMsg("Error: Badly formed Style property, need to be a scalar, sequence or map. %s will not be added to "
                   "stype properties.\n",
                   paramKey.c_str());
        }
    }
