...
DirectionalLight::DirectionalLight(const std::string& _name, bool _dynamic)
    : Light(_name, _dynamic), m_direction(1.0, 0.0, 0.0) {
...
void DirectionalLight::setDirection(const glm::vec3& _dir) { m_direction = glm::normalize(_dir); }
...
void DirectionalLight::setupProgram(const std::shared_ptr<View>& _view, std::shared_ptr<ShaderProgram> _shader) {
    ...
    if (m_origin == LightOrigin::world) { direction = _view->getNormalMatrix() * direction; }
...
std::string DirectionalLight::getClassBlock() {
    if (s_classBlock.empty()) { s_classBlock = stringFromResource("directionalLight.glsl") + "\n"; }
    return s_classBlock;
}
...
std::string DirectionalLight::getInstanceAssignBlock() {
    ...
    if (!m_dynamic) { block += ", " + glm::to_string(m_direction) + ")"; }
    return block;
}

const std::string& DirectionalLight::getTypeName() { return s_typeName; }
