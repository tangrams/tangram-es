...
PointLight::PointLight(const std::string& _name, bool _dynamic)
    : Light(_name, _dynamic), m_position(0.0), m_attenuation(0.0), m_innerRadius(0.0), m_outerRadius(0.0) {
...
void PointLight::setAttenuation(float _att) { m_attenuation = _att; }
...
void PointLight::setupProgram(const std::shared_ptr<View>& _view, std::shared_ptr<ShaderProgram> _shader) {
    if (m_dynamic) {
        ...
        if (m_attenuation != 0.0) { _shader->setUniformf(getUniformName() + ".attenuation", m_attenuation); }

        if (m_innerRadius != 0.0) { _shader->setUniformf(getUniformName() + ".innerRadius", m_innerRadius); }

        if (m_outerRadius != 0.0) { _shader->setUniformf(getUniformName() + ".outerRadius", m_outerRadius); }
    }
}

std::string PointLight::getClassBlock() {
    if (s_classBlock.empty()) { s_classBlock = stringFromResource("pointLight.glsl") + "\n"; }
    return s_classBlock;
}
...
std::string PointLight::getInstanceDefinesBlock() {
    ...
    if (m_attenuation != 0.0) { defines += "#define TANGRAM_POINTLIGHT_ATTENUATION_EXPONENT\n"; }

    if (m_innerRadius != 0.0) { defines += "#define TANGRAM_POINTLIGHT_ATTENUATION_INNER_RADIUS\n"; }

    if (m_outerRadius != 0.0) { defines += "#define TANGRAM_POINTLIGHT_ATTENUATION_OUTER_RADIUS\n"; }
    return defines;
}
...
std::string PointLight::getInstanceAssignBlock() {
    ...
    if (!m_dynamic) {
        ...
        if (m_attenuation != 0.0) { block += ", " + std::to_string(m_attenuation); }
        if (m_innerRadius != 0.0) { block += ", " + std::to_string(m_innerRadius); }
        if (m_outerRadius != 0.0) { block += ", " + std::to_string(m_outerRadius); }
        block += ")";
    }
    return block;
}

const std::string& PointLight::getTypeName() { return s_typeName; }
