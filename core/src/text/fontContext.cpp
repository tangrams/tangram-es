...
// TODO: make this platform independent
    ...
    int size = BASE_SIZE;
    for (int i = 0; i < 3; i++, size += STEP_SIZE) {
        m_font[i] = m_alfons.addFont("default", alfons::InputSource(resolvePath(DEFAULT, PathType::internal)), size);
        m_font[i]->addFace(m_alfons.addFontFace(alfons::InputSource(resolvePath(FONT_AR, PathType::internal)), size));
        m_font[i]->addFace(m_alfons.addFontFace(alfons::InputSource(resolvePath(FONT_HE, PathType::internal)), size));
        m_font[i]->addFace(m_alfons.addFontFace(alfons::InputSource(resolvePath(FONT_JA, PathType::internal)), size));
        m_font[i]->addFace(m_alfons.addFontFace(alfons::InputSource(resolvePath(FALLBACK, PathType::internal)), size));
    }
#endif
}
