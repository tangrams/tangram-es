...
FontContext::~FontContext() { glfonsDelete(m_fsContext); }

std::shared_ptr<TextBuffer> FontContext::genTextBuffer() {
    return std::shared_ptr<TextBuffer>(new TextBuffer(m_fsContext));
}

const std::unique_ptr<Texture>& FontContext::getAtlas() const { return m_atlas; }

void FontContext::getProjection(float* _projectionMatrix) const { glfonsProjection(m_fsContext, _projectionMatrix); }

void FontContext::setScreenSize(int _width, int _height) { glfonsScreenSize(m_fsContext, _width, _height); }

void FontContext::clearState() { fonsClearState(m_fsContext); }

void FontContext::useBuffer(const std::shared_ptr<TextBuffer>& _textBuffer) {
    if (_textBuffer) { _textBuffer->bind(); }
...
void FontContext::lock() { m_contextMutex->lock(); }

void FontContext::unlock() { m_contextMutex->unlock(); }

std::shared_ptr<TextBuffer> FontContext::getCurrentBuffer() { return m_currentBuffer.lock(); }

bool FontContext::addFont(const std::string& _fontFile, std::string _name) {
    if (m_fonts.find(_name) != m_fonts.end()) { return true; }
...
void FontContext::setFont(const std::string& _name, int size) {
    ...
    } else { logMsg("[FontContext] Could not find font %s\n", _name.c_str()); }
}

void updateAtlas(void* _userPtr, unsigned int _xoff, unsigned int _yoff, unsigned int _width, unsigned int _height,
                 const unsigned int* _pixels) {
...
void FontContext::initFontContext(int _atlasSize) {
    ...
    m_fsContext =
        glfonsCreate(_atlasSize, _atlasSize, FONS_ZERO_TOPLEFT, {false, updateBuffer, updateAtlas}, (void*)this);
}
